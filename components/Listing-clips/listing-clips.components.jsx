...
function ListingClipsComponents({ videosData, setVideosData }) {
  ...
  const formatTimeAgo = (creationDate) => {
    ...
    if (differenceInDaysValue > 7) {
      return `${Math.floor(differenceInDaysValue / 7)} week${Math.floor(
        differenceInDaysValue / 7
      ) === 1 ? "" : "s"} ago`;
    } else if (differenceInDaysValue > 0) {
      return `${differenceInDaysValue} day${differenceInDaysValue === 1 ? "" : "s"} ago`;
    }

    const differenceInHoursValue = differenceInHours(now, new Date(creationDate));

    if (differenceInHoursValue > 0) {
      return `${differenceInHoursValue} hour${differenceInHoursValue === 1 ? "" : "s"} ago`;
    }
    ...
    return `${differenceInMinutesValue} minute${differenceInMinutesValue === 1 ? "" : "s"} ago`;
  };
                ...
                <div className="video-clip">
                  <div className="video-display">
                    ...
                    <LazyImage
                      ...
                      className="lazy_img video_thumbnail rd10"
                    />
                    <Modal
